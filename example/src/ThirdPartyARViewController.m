...
static CGFloat ROUTE_ELEVATION_FROM_FLOOR_M = 0.5;
static CGFloat BREADCRUMB_DISTANCE_M = 1.0;

@interface IALatLngFloor (CLLocation)
@property (nonatomic, assign, readonly) CLLocationCoordinate2D coordinate;
@end

@implementation IALatLngFloor (CLLocation)
- (CLLocationCoordinate2D)coordinate {
    return CLLocationCoordinate2DMake(self.latitude, self.longitude);
}
@end

@interface RouteBreadCrumb : NSObject
@property (nonatomic, strong) IALatLngFloor* position;
@property (nonatomic, assign) CLLocationDirection heading;
@property (nonatomic, assign) CLLocationDistance elevation;
@property (nonatomic, strong) SCNNode* node;
@end

@implementation RouteBreadCrumb
- (instancetype)initWithPosition:(IALatLngFloor*)position heading:(CLLocationDirection)heading elevation:(CLLocationDistance)elevation {
    if (self = [super init]) {
        self.position = position;
        self.heading = heading;
        self.elevation = elevation;
        
        self.node = deepCopyNode([SCNScene sceneNamed:@"Models.scnassets/arrow_stylish.obj"].rootNode.childNodes.firstObject);
        self.node.scale = SCNVector3Make(0.1, 0.1, 0.1);
        SCNMaterial* material = [SCNMaterial material];
        material.diffuse.contents = [UIColor colorWithRed:95.0/255.0 green:209.0/255.0 blue:195.0/255.0 alpha:1.0];
...
@interface ThirdPartyARViewController () <IALocationManagerDelegate, ARSCNViewDelegate, ARSessionDelegate, UISearchBarDelegate, UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, copy) NSArray<RouteBreadCrumb*>* routeBreadCrumbs;
@end

@implementation ThirdPartyARViewController
...
- (void)indoorLocationManager:(IALocationManager *)manager didUpdateRoute:(IARoute *)route {
    ...
    NSMutableArray<RouteBreadCrumb*>* newRouteBreadCrumbs = [NSMutableArray array];
    CGFloat pathLength = BREADCRUMB_DISTANCE_M;
    for (IARouteLeg* leg in route.legs.reverseObjectEnumerator) {
        if (leg.length < 1e-6) {
            continue;
        }
        
        // this part is a bit tricky / ugly
        IARoutePoint* e = leg.begin;
        IARoutePoint* b = leg.end;
        simd_float4x4 tmpMat = [manager.arSession geoToAr:b.coordinate floorNumber:(int)b.floor heading:0 zOffset:0];
        CGFloat y0 = tmpMat.columns[3][1];
        tmpMat = [manager.arSession geoToAr:e.coordinate floorNumber:(int)e.floor heading:0 zOffset:0];
        CGFloat floorHeight = tmpMat.columns[3][1] - y0;
        while (pathLength < leg.length) {
            CGFloat s = pathLength / leg.length;
            RouteBreadCrumb* rbc = [[RouteBreadCrumb alloc] initWithPosition:[IALatLngFloor latLngFloorWithLatitude:(1 - s) * b.coordinate.latitude + s * e.coordinate.latitude andLongitude:(1 - s) * b.coordinate.longitude + s * e.coordinate.longitude andFloor:b.floor] heading:leg.direction elevation:ROUTE_ELEVATION_FROM_FLOOR_M + s * floorHeight];
            [newRouteBreadCrumbs addObject:rbc];
    ...
    self.routeBreadCrumbs = newRouteBreadCrumbs;
}
...
- (void)renderer:(id<SCNSceneRenderer>)renderer willRenderScene:(SCNScene *)scene atTime:(NSTimeInterval)time {
    ...
    if (arSession.converged == YES) {
        ...
        for (RouteBreadCrumb* rbc in self.routeBreadCrumbs) {
            simd_float4x4 modelMatrix = [arSession geoToAr:rbc.position.coordinate floorNumber:(int)rbc.position.floor heading:0 zOffset:0];
            rbc.node.simdWorldTransform = modelMatrix;
            rbc.node.eulerAngles = SCNVector3Make(-M_PI/2.0, 0, 0);
...
- (void)session:(ARSession *)session didUpdateFrame:(ARFrame *)frame {
    ...
    if (arSession.converged == YES) {
        ...
        for (RouteBreadCrumb* rbc in self.routeBreadCrumbs) {
            [self.arView.scene.rootNode addChildNode:rbc.node];
        }
